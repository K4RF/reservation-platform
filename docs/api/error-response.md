...
| `409 Conflict` | 현재 리소스 또는 상태와 요청이 충돌함 | 이메일 중복, 객실 재고 부족, 비활성 리소스 예약, 잘못된 상태 전이 |
...
| `ACCOMMODATION_003` | 400 | 숙소 통합 검색의 누락·역전된 기간 |
| `ACCOMMODATION_004` | 400 | 숙소 통합 검색의 객실 기본 가격 범위 오류 |
| `ACCOMMODATION_005` | 400 | 기간 없이 숙소 예약 가능 여부 검색 |
| `BOOKING_POLICY_001` | 404 | 수정할 숙소 예약 정책 미존재 |
| `BOOKING_POLICY_002` | 409 | 숙소 예약 정책 중복 |
| `BOOKING_POLICY_003` | 400 | 최소·최대 숙박일 범위 오류 |
| `BOOKING_POLICY_004` | 400 | 최소·최대 사전 예약일 범위 오류 |
| `BOOKING_POLICY_005` | 409 | 최소 숙박일 미충족 |
...
| `RESERVATION_002` | 409 | 과거 기간 중복 검증 오류(재고 기반 처리 전환 후 신규 응답에는 사용하지 않음) |
...
| Code | Status | Meaning |
| --- | --- | --- |
| `INVENTORY_001` | 404 | 숙박일 중 날짜별 객실 재고 누락 |
| `INVENTORY_002` | 409 | 동일 객실·날짜 재고 중복 |
| `INVENTORY_003` | 400 | 음수 전체 재고 |
| `INVENTORY_004` | 400 | 1 미만 증감 수량 |
| `INVENTORY_005` | 409 | 숙박일 중 하나 이상의 잔여 재고 부족 |
...
| Code | Status | Meaning |
| --- | --- | --- |
| `ROOM_PRICE_001` | 404 | 수정할 날짜별 객실 가격 미존재 |
| `ROOM_PRICE_002` | 409 | 동일 객실·날짜 가격 중복 |
| `ROOM_PRICE_003` | 400 | 0 이하인 날짜별 객실 가격 |
